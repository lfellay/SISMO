...
are located and labelled by a Sturm-type determinant-sign scan with no
asymptotic input, and the well-known inaccuracy of split schemes for dipole
...
two-degree spectrum is obtained in $6.6\,$s on 18 threads, about eleven times
...
resonance structure of the operator rather than on asymptotic estimates, and
...
\begin{equation}
  ...
  S_6=A_{\rm osc},
\end{equation}
where $m(x)$ is the fractional mass inside $x$.  Two combinations occur repeatedly: the local dimensionless gravity is $g=S_2x$, and the squared Brunt--V\"ais\"al\"a (buoyancy) frequency is $N^2=S_2S_6x^2$.  The four dependent variables are the scaled radial displacement, horizontal displacement, potential perturbation, and potential gradient:
\begin{align}
...
\begin{equation}
  ...
  \label{eq:phABC}
\end{equation}
where $A$ is a constant classically set by the vacuum condition at the surface (the interior potential must match a decaying exterior solution),
\begin{equation}
  y_4+(\ell+1)\,y_3=-\frac{S_4}{S_2}\,y_1 \qquad (x=x_{\rm s}).
  \label{eq:surfBC}
\end{equation}
\\~\\For $\ell\ge2$ this closure works well.  For the dipole it can become problematic.  The homogeneous solution grows outward as $x^{\ell}$, through the envelope and the model atmosphere; the amplitude $A$ is fixed by a condition imposed at the very top of that growth; and the resulting potential is then used deep in the interior, where, for $\ell=1$ specifically,  it has a strong effect on the frequency.  A small inaccuracy at the surface is thus propagated globally and amplified.  \autoref{subsec_dipole_closure} replaces this closure for $\ell=1$ by a condition that involves the whole star instead of one point.
...
value to within $7\times10^{-4}$. This represents an improvement over the response-based scans used in the
...
order-of-magnitude reduction in wall-clock time results from three main
...
formulation. This low cost makes the direct Sturm scan practical: approximately
$10^4$ factorisations of the complete 24,992-point matrix are performed in
about two seconds.
...
wall-clock time decreases from approximately $104$,s in serial execution to
$6.6$,s when using 18 threads. Third, the calculation is performed on the original model grid throughout.
...
the fundamental -- at roughly one tenth of the computational cost.  The three
